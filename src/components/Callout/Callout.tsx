...
import { calloutBodyStyles, calloutTitleStyles, calloutVariantMap } from './Callout.styles';
import type { CalloutBodyProps, CalloutMainProps, CalloutVariant } from './Callout.types';
...
const useCalloutContext = () => {
  ...
  if (!context) {
    throw new Error('서브 컴포넌트는 메인 컴포넌트 내부에서만 사용 가능합니다.');
  }
...
const CalloutMain = ({ variant, title, children }: CalloutMainProps) => {
  return (
    <CalloutContext.Provider value={{ variant }}>
      <div className={`flex flex-col gap-2 rounded-lg border p-4 ${calloutVariantMap[variant]}`}>
...
const CalloutList = ({ children }: CalloutBodyProps) => {
  ...
  return (
    <ul className={`pl-4 space-y-1 marker:opacity-60 ${calloutBodyStyles[variant]}`}>{children}</ul>
  );
};
